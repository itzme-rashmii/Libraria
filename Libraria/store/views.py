...
def signin(request):
    if request.user.is_authenticated:
        return redirect('store:index')  # Redirect to the index page if already logged in
    else:
        if request.method == "POST":
            user = request.POST.get('user')  # Get the username from the POST data
            password = request.POST.get('pass')  # Get the password from the POST data
            auth = authenticate(request, username=user, password=password)  # Authenticate the user
            if auth is not None:
                login(request, auth)  # Log the user in if authentication is successful
                return redirect('store:index')  # Redirect to the index page
            else:
                messages.error(request, "Username and password don't match.")  # Show error message
